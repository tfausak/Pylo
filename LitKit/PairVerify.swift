...
enum PairVerifyHandler {
    ...
    private static let logger = Logger(subsystem: "com.example.hap", category: "PairVerify")
    ...
    static func handle(request: HTTPRequest, connection: HAPConnection, server: HAPServer) -> HTTPResponse {
        ...
        guard let stateData = tlv[.state],
              let state = stateData.first else {
            return errorResponse(.unknown)
        }
    ...
    private static func handleM1(tlv: [TLV8.Tag: Data], connection: HAPConnection, server: HAPServer) -> HTTPResponse {
        guard let controllerPublicKeyData = tlv[.publicKey],
              controllerPublicKeyData.count == 32 else {
            return errorResponse(.unknown)
        }

        do {
            let controllerPublicKey = try Curve25519.KeyAgreement.PublicKey(rawRepresentation: controllerPublicKeyData)
            ...
            let sharedSecret = try accessoryPrivateKey.sharedSecretFromKeyAgreement(with: controllerPublicKey)
    ...
    private static func handleM3(tlv: [TLV8.Tag: Data], connection: HAPConnection, server: HAPServer) -> HTTPResponse {
        guard let encryptedData = tlv[.encryptedData],
              ...
              let accessoryPrivateKey = session.accessoryEphemeralPrivateKey else {
        ...
        do {
            ...
            guard let controllerIdentifier = subTLV[.identifier],
                  let controllerSignature = subTLV[.signature] else {
                return errorResponse(.authentication)
            }
